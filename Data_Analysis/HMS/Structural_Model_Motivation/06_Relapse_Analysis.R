...
# February 2026
#
# This script conducts a relapse analysis that tracks households who stop
# purchasing tobacco ("quit episodes") and measures months until they restart.
# Under exponential discounting, a household that finds it optimal to stop
# should remain stopped absent a sufficiently large shock. Under present bias,
# the household may plan to stay quit but re-optimize when tomorrow becomes
# today, generating systematic relapse. High relapse rates -- especially
# rapid ones -- are consistent with beta < 1.
#
...
# Takes a data.table and a purchase indicator column name. Returns a list with:
...
# Compute the empirical CDF of months to relapse for cigarettes and e-cigarettes.
...
n_cig  <- nrow(dt_cdf_cig)
n_ecig <- nrow(dt_cdf_ecig)
...
dt_cdf[, cdf_cig  := sapply(months_to_relapse, function(h) sum(dt_cdf_cig$months_to_relapse <= h) / n_cig)]
dt_cdf[, cdf_ecig := sapply(months_to_relapse, function(h) sum(dt_cdf_ecig$months_to_relapse <= h) / n_ecig)]
