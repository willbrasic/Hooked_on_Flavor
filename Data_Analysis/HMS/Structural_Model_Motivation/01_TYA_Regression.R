...
# September 2025
#
# This script estimates the effect of teens and young adults in the household
...
# Different LPM RHS variables
vars_fda <- list(
  teen_present = list(label = "HH Contains a Teen", table = "fda_teen_regression"),
  young_adult_present = list(label = "HH Contains a Young Adult", table = "fda_youngadult_regression"),
  teen_or_young_adult_present = list(label = "HH Contains a Teen or Young Adult", table = "fda_teen_youngadult_regression")
)

# Loop over each RHS variable
for (var in names(vars_fda))
{
  ...
  model_1 <- plm(formula_1, index = c("household_code", "purchase_month"), model = "pooling", data = dt_complete_fda)

  # (2) Add household size
  formula_2 <- as.formula(paste0("flavored_ecig ~ fda_authorized_ecig * ", var, " + household_size + i(purchase_month)"))
  model_2 <- plm(formula_2, index = c("household_code", "purchase_month"), model = "pooling", data = dt_complete_fda)

  # (3) Add state FE
  formula_3 <- as.formula(paste0("flavored_ecig ~ fda_authorized_ecig * ", var, " + household_size + i(fips_state_code) + i(purchase_month)"))
  model_3 <- plm(formula_3, index = c("household_code", "purchase_month"), model = "pooling", data = dt_complete_fda)
  ...
  stargazer(
    ...
    dep.var.labels = "Purchased Flavored E-Cigarette",
    column.labels = c("(1)", "(2)", "(3)"),
    covariate.labels = c("FDA Authorized E-Cig", vars_fda[[var]]$label,
                         paste0("FDA Authorized $\\times$ ", vars_fda[[var]]$label), "HH Size"),
    omit.stat = c("f", "ser"),
    add.lines = add_lines,
    keep.stat = c("n", "rsq"),
    no.space = TRUE,
    digits = 3,
    label = vars_fda[[var]]$table
  )
}
...
# Keep only e-cigarette purchase observations
dt_ecig_fda <- dt[ecig == 1 | cig_ecig == 1]

# Restrict to complete cases on required variables
dt_complete_fda_ecig <- dt_ecig_fda[complete.cases(dt_ecig_fda[, ..vars_all_fda])]

# Different LPM RHS variables
vars_fda_ecig <- list(
  teen_present = list(label = "HH Contains a Teen", table = "fda_ecig_teen_regression"),
  young_adult_present = list(label = "HH Contains a Young Adult", table = "fda_ecig_youngadult_regression"),
  teen_or_young_adult_present = list(label = "HH Contains a Teen or Young Adult", table = "fda_ecig_teen_youngadult_regression")
)

# Loop over each RHS variable
for (var in names(vars_fda_ecig))
{
  # (1) Baseline: fda_authorized_ecig + var + interaction + month FE
  formula_1 <- as.formula(paste0("flavored_ecig ~ fda_authorized_ecig * ", var, " + i(purchase_month)"))
  model_1 <- plm(formula_1, index = c("household_code", "purchase_month"), model = "pooling", data = dt_complete_fda_ecig)

  # (2) Add household size
  formula_2 <- as.formula(paste0("flavored_ecig ~ fda_authorized_ecig * ", var, " + household_size + i(purchase_month)"))
  model_2 <- plm(formula_2, index = c("household_code", "purchase_month"), model = "pooling", data = dt_complete_fda_ecig)

  # (3) Add state FE
  formula_3 <- as.formula(paste0("flavored_ecig ~ fda_authorized_ecig * ", var, " + household_size + i(fips_state_code) + i(purchase_month)"))
  model_3 <- plm(formula_3, index = c("household_code", "purchase_month"), model = "pooling", data = dt_complete_fda_ecig)
  ...
  stargazer(
    ...
    dep.var.labels = "Purchased Flavored E-Cigarette",
    column.labels = c("(1)", "(2)", "(3)"),
    covariate.labels = c("FDA Authorized E-Cig", vars_fda_ecig[[var]]$label,
                         paste0("FDA Authorized $\\times$ ", vars_fda_ecig[[var]]$label), "HH Size"),
    omit.stat = c("f", "ser"),
    add.lines = add_lines,
    keep.stat = c("n", "rsq"),
    no.space = TRUE,
    digits = 3,
    label = vars_fda_ecig[[var]]$table
  )
}
