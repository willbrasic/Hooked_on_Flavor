...
Uses flush() to force Julia to print to the terminal and write to the file
immediately rather than buffering. log_io is a global defined at the top of
this script, so it does not need to be passed as an argument.
"""
...
function get_price_ratios(
    ...
    # j=35:46: bundles, mapped to the closest standalone bin's ratio by consumption
    for j in idx:N_J
...
function addiction_evolution(
    ...
    # ã' = (1-ψ)·ã + ψ·n_std
    # `a` here is already ã, so no additional ψ multiplication on the decay term.
    return (1 - ψ) * a + ψ * n
end
...
function simulate_addiction_trajectories(
    ...
    for i in 1:N
        ...
        # Use the household's current addiction level if it exists in a_current,
        # otherwise fall back to the pre-estimated initial stock from a₀[hh]
        # (first observation for this household).
...
During VFI, the last iteration's V_choice values are computed from V_now of the
PREVIOUS iteration. After convergence, V_now has been updated to the final
converged values, but V_choice was built from the second-to-last V_now.
This function recomputes V_choice one final time using the converged V_now,
so the returned choice-specific values are exactly consistent with the
...
where e_d is the d-th standard basis vector. Without the add offsets,
all vertices would be identical and Nelder-Mead could not move toward the
optimum.
