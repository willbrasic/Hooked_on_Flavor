...
# December 2025
#
# This script uses Halton draws to simulate the state-transitions
################################################################################
...
# Load in AR parameters
file_name_Phi <- paste0("./AR_Parameters/AR_Parameters_Phi.csv")
file_name_Sigma_hat <- paste0("./AR_Parameters/AR_Parameters_Sigma.csv")
...
file_name <- paste0("./Pricing_Spaces.csv") 
...
# All |P|^|K| index combinations (cig × ecig)
combinations <- CJ(
...
# Essentially this code first gets u_r ~ U(0, 1) using K prime bases
# and then uses the inverse CDF of the normal distribution (normal = TRUE) to turn those u_r
# into z_r ~ N(0, I_K)
# Given there are K = 2 categories, the prime base for cigs is 2
# and the prime base for e-cigs is 3
...
# Write correlated shocks (L * z) to a file
file_name <- "../Dynamic_Model/Halton_Draw_Shocks.csv"
