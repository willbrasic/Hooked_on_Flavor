...
#
# Three ban types are evaluated (apply_flavor_ban!, apply_fda_flavor_ban!,
# apply_non_fda_ban!), plus a flavor tax counterfactual (apply_flavor_tax!).
#
# For each ban type / tax level and β in BETA_GRID: solve VFI under SQ and
# the policy, compute posterior type probabilities and pointwise choice
# probs/welfare, then forward-simulate households via Monte Carlo
# (N_draws=100, T_sim=36 months) using common random numbers so SQ-vs-policy
# differences reflect the policy itself, not simulation noise. Results are
# aggregated overall and by TYA status / latent type.
################################################################################
...
# β is read from ENV; fixed per run, never estimated.
...
using Random  # CRN seeding for forward simulations
...
if HPC
    ...
    # Construct psi, beta, and psi_3 tags for directory and file naming 
    ...
    # Output path is β-specific so each β value gets its ownresults directory.
    output_dir = "/home/u2/wbrasic/4th_Year_Paper/Dynamic_Model/04_CF_Mixture_V2/CF_Mixture_$(psi_tag)_$(psi_3_tag)_Beta_$(beta_tag)_Results"
    ...
    # Output path is β-specific so each β value gets its own top-level results directory.
    output_dir = "C:/Users/wbras/OneDrive/Documents/Desktop/UA/4th_Year_Paper/4th_Year_Paper_Data/HMS/2021-Onward/Dynamic_Model/CF_Mixture_$(psi_tag)_$(psi_3_tag)_Beta_$(beta_tag)_Results"
...
# Structural parameters and PSI_3 differ by β (each was estimated separately),
# so objects dependent on β are loaded and pre-computed once per β in section 13.
...
# The flavored habit stock depends on PSI_3, which differs by β, so it is
# built per β inside the pre-loading loop in section 13.
...
# Flavored habit stock depends on PSI_3 (β-specific); built per β in section 13
...
# Single-element β grid so the pre-loading loop and all counterfactual loops
# iterate exactly once, over the β value read from ENV in section 1.
BETA_GRID = [BETA_VAL]
...
# Pre-computes all β-specific objects (params, mixing weights, flow utilities,
# flavored-habit objects) once per β in BETA_GRID and stores them in beta_params[β].
...
# All variates are drawn ONCE, before any counterfactual loop, and reused across
# every β and policy scenario. SQ and counterfactual paths consume the exact
# same draws in the same order (CRN), so outcome differences reflect the
# policy itself rather than simulation noise.
...
for (ban_label, ban_fn!) in BAN_TYPES
    ...
    for beta_val in BETA_GRID
        ...
        beta_subdir = ban_subdir  # β is encoded in output_dir; no separate Beta_* subdir
        mkpath(beta_subdir)
        ...
        # Mixture-weighted: P̂(j|i) = Σ_l P̂(l|y_h) · P(j|l, x_i); integrates out type
        # uncertainty rather than hard-assigning each HH to its modal type.
        ...
        # Each draw samples a latent type from the household's posterior, then
        # simulates T_sim periods from the household's terminal state under both
        # SQ and ban, sharing the pre-drawn CRN variates (see section 14).
        ...
        # Parallelized over households; interleaving SQ and ban within the same
        # (h, d, t) iteration keeps both scenarios consuming CRN draws in lockstep.
        Threads.@threads for h in 1:N_HH
            ...
            for d in 1:N_draws
                ...
                # Inverse-CDF type draw from the posterior: u <= w1 -> type 1;
                # u <= w1+w2 -> type 2; else type 3
                u_type = crn_type[h, d]
                ...
                for t in 1:T_sim
                    ...
                    # flavor ban (same steps, ban's V_decision, same CRN draws as SQ)
        ...
        # Aggregate TYA-present households by modal latent type (K=3 modal assignment)
        ...
        open(sim_results_path, "w") do io
            ...
            for t in 1:T_sim
                row = [@sprintf("%d", t),
                    ...
                    @sprintf("%.10f", agg_sq.mean_q_cig[t]),
                    @sprintf("%.10f", agg_sq.mean_q_orig_ecig[t]),
                    @sprintf("%.10f", agg_sq.mean_q_flav_ecig[t]),
                    ...
                    @sprintf("%.10f", agg_ban.mean_q_cig[t]),
                    @sprintf("%.10f", agg_ban.mean_q_orig_ecig[t]),
                    @sprintf("%.10f", agg_ban.mean_q_flav_ecig[t])]
                println(io, join(row, ","))
            end
        end
            ...
            for (group_label, agg_sq_g, agg_ban_g) in [("tya", agg_sq_tya, agg_ban_tya)]
                for t in 1:T_sim
                    row = [group_label, @sprintf("%d", t),
                        ...
                        @sprintf("%.10f", agg_sq_g.mean_q_cig[t]),
                        @sprintf("%.10f", agg_sq_g.mean_q_orig_ecig[t]),
                        @sprintf("%.10f", agg_sq_g.mean_q_flav_ecig[t]),
                        ...
                        @sprintf("%.10f", agg_ban_g.mean_q_cig[t]),
                        @sprintf("%.10f", agg_ban_g.mean_q_orig_ecig[t]),
                        @sprintf("%.10f", agg_ban_g.mean_q_flav_ecig[t])]
                    println(io, join(row, ","))
                end
            end
        end
        ...
        sim_tya_type_path = joinpath(beta_subdir, "Simulation_by_TYA_Type.csv");
        ...
        for (thresh, suffix) in [(0.05, "_thresh005"), (0.10, ""), (0.20, "_thresh020")]
            ...
            ext_path_t = joinpath(beta_subdir, "Extensive_Margin_by_Type$(suffix).csv")
            open(ext_path_t, "w") do io
        ...
        for (thresh, suffix) in [(0.05, "_thresh005"), (0.10, ""), (0.20, "_thresh020")]
            ...
            ext_path_t = joinpath(beta_subdir, "Extensive_Margin_by_TYA_Type$(suffix).csv")
            open(ext_path_t, "w") do io
                header = ["type", "period", "n_non_users",
                          "sq_ever_initiated", "cf_ever_initiated", "prevention_rate"]
                println(io, join(header, ","))
...
for tau in TAX_GRID
    ...
    for beta_val in BETA_GRID
        ...
        local beta_tag = numeric_tag(beta_val)
        beta_subdir = joinpath(tax_subdir, tau_subdir_name)  # β encoded in output_dir
        mkpath(beta_subdir)
        ...
        # Declare loop-local variables to avoid Julia warnings
        ...
        # Log the utility shift for the median flavored alternative. Raw mL is
        # shown for interpretability; the utility shift itself uses the
        # standardized quantity, matching apply_flavor_tax!.
        ...
        # Compute household-level posterior type probabilities 
        hh_posterior_type1 = Vector{Float64}(undef, N_HH)
        hh_posterior_type2 = Vector{Float64}(undef, N_HH)
        hh_posterior_type3 = Vector{Float64}(undef, N_HH)
        ...
        # Each draw samples a latent type from the household's posterior, then
        # simulates T_sim periods from the household's terminal state under both
        # SQ and tax, sharing the pre-drawn CRN variates (see section 14).
        ...
        Threads.@threads for h in 1:N_HH
            ...
            for d in 1:N_draws
                ...
                for t in 1:T_sim
                    ...
                    # flavor tax (same steps, taxed V_decision, same CRN draws as SQ)
                    ...
                    sim_welfare_tax_arr[h, t, d] = interpolate_v_now(
                        V_now_tax, tya_tax, a_f_tax, a_s_tax, a_flav_tax, p_cig_tax, p_ecig_tax,
                        N_P, A_f, A_s, A_flav, P
                    )
...
# For each β, find the per-mL tax τ* that matches the comprehensive ban's mean
# addiction reduction (36-month average, all households), then compare welfare costs.
...
for beta_val in BETA_GRID
    ...
    ban_overall_path = joinpath(output_dir, "Ban_Comprehensive", "Simulation_Overall.csv")
    ...
    sq_mean_welfare    = mean(df_ban_all.sq_welfare)
    ...
    for tau in TAX_GRID
        ...
        tax_overall_path = joinpath(output_dir, "Flavor_Tax", "Tax_$tau_tag", "Simulation_Overall.csv")
        ...
        tax_sq_mean_welfare = mean(df_tax_all.sq_welfare)
        tax_welfare_effect = tax_mean_welfare - tax_sq_mean_welfare
...
if !HPC
    ...
    blabels  = [numeric_tag(b) for b in BETA_GRID]   # single-element with current β
    ref_tag  = beta_tag                                # current β is the reference
    T_fig    = 36

    # Load Simulation_by_TYA.csv for a given policy/beta, restricted to TYA-present households
    ...
    # comp[bl][grp], fda[bl][grp], tax[tau_tag][bl][grp]  where grp = "tya" (TYA-present only)
    ...
    for bl in blabels
        p_comp = joinpath(output_dir, "Ban_Comprehensive", "Simulation_by_TYA.csv")
        p_fda  = joinpath(output_dir, "Ban_FDA_Only",     "Simulation_by_TYA.csv")
        if isfile(p_comp)
            t1 = load_tya_split(p_comp)
            comp[bl] = Dict("tya" => t1)
        end
        if isfile(p_fda)
            t1 = load_tya_split(p_fda)
            fda[bl] = Dict("tya" => t1)
        end
    end
    ...
    for tau in TAX_GRID
        ...
        for bl in blabels
            p_tax = joinpath(output_dir, "Flavor_Tax", "Tax_$tau_tag", "Simulation_by_TYA.csv")
            if isfile(p_tax)
                t1 = load_tya_split(p_tax)
                tax[tau_tag][bl] = Dict("tya" => t1)
            end
        end
    end
